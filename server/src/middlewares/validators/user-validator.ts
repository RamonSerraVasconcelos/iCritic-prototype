...
const validateRegister = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!hasValueMinLength(password, 8)) {
        throw new ResponseError('Password must contain 8 characters minimum!', 400);
    }

    if (!hasValueMaxLength(password, 175)) {
        throw new ResponseError('Password must contain 75 characters maximum!', 400);
    }

    if (description) {
        if (!hasValueMinLength(description, 4)) {
            throw new ResponseError('Description must contain 4 characters minimum!', 400);
        }

        if (!hasValueMaxLength(description, 400)) {
            throw new ResponseError('Description must contain 400 characters maximum!', 400);
        }
    }

    if (countryId && !validateNumber(countryId)) {
        throw new ResponseError('Invalid country id format', 400);
    }
...
const validateUserUpdate = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (isEmpty(id) || !validateNumber(id)) {
        throw new ResponseError('Invalid id format', 400);
    }

    if (name) {
        if (!hasValueMinLength(name, 2)) {
            throw new ResponseError('Username must contain 2 characters minimum!', 400);
        }
